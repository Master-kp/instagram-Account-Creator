...
class OneSecMail:
    ...
    def __init__(
        ...
        self.username = username or random_string(10)
    ...
    def get_inbox(self) -> List['OneSecMail.MessageInfo']:
        ...
        resp = self._session.get(
            f'https://www.1secmail.com/api/v1/?action=getMessages&login={self.username}&domain={self.domain}')
    ...
    def get_message(self, id: int) -> 'OneSecMail.Message':
        ...
        resp = self._session.get(
            f'https://www.1secmail.com/api/v1/?action=readMessage&login={self.username}&domain={self.domain}&id={id}')
        resp.raise_for_status()
    ...
    def download_attachment(self, id: int, file: str) -> bytes:
        ...
        resp = self._session.get(
            f'https://www.1secmail.com/api/v1/?action=download&login={self.username}&domain={self.domain}&id={id}&file={file}')
        resp.raise_for_status()
        return resp.content
    ...
    class Attachment:
        ...
        @classmethod
        def from_dict(cls, mail_instance: 'OneSecMail', message_id: int,
                      attachment: Dict[str, any]) -> 'OneSecMail.Attachment':
...
def filter_numbers(to_filter):
    return ''.join(re.findall(r'\d+', to_filter))
